...
def seed_data():
    ...
    # 嘗試多種可能的路徑來讀取 CSV 文件
    possible_paths = [
        '../tests/gov.csv',        # 相對於 app 目錄
        'tests/gov.csv',           # 相對於項目根目錄
        '/app/tests/gov.csv',      # Docker 容器中的絕對路徑
        os.path.join(os.path.dirname(__file__), '../tests/gov.csv')  # 基於當前文件的相對路徑
    ]
    
    csv_loaded = False
    for csv_path in possible_paths:
        print(f"嘗試路徑: {csv_path}")
        if os.path.exists(csv_path):
            ...
            try:
                ...
                if CompanyGov.query.count() == 0:
                    ...
                    for index, row in df.iterrows():
                        try:
                            ...
                            print(f"添加公司: {company_gov.company_name}")
                ...
                print(f"讀取 CSV 文件 {csv_path} 時出錯: {e}")
    
    if not csv_loaded:
        print("無法讀取任何 CSV 文件，使用默認數據")
        
        # 如果找不到 CSV 文件或讀取失敗，則添加一個示例政府公司資料
        if CompanyGov.query.count() == 0:
            print("添加示例政府公司資料...")
            company_govs = [
                CompanyGov(
